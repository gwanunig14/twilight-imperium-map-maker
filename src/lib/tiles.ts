...
function normalizeTile(id: string): SystemTile | null {
  ...
  const resources = planets.reduce(
    (sum: number, planet: any) => sum + planet.resources,
    0,
  );
  const influence = planets.reduce(
    (sum: number, planet: any) => sum + planet.influence,
    0,
  );
  const optimalSpend = planets.reduce(
    (sum: number, planet: any) =>
      sum + Math.max(planet.resources, planet.influence),
    0,
  );
  const planetCount = planets.length;
  const specialtyIcons = Object.values(specialtyByType).reduce(
    (sum, value) => sum + value,
    0,
  );
...
export function featureCount(tile: SystemTile, key: FeatureKey): number {
  if (key.startsWith("specialty:"))
    return tile.specialtyByType[key.slice("specialty:".length)] ?? 0;
  if (key.startsWith("anomaly:"))
    return tile.anomalies.filter(
      (value) => value === key.slice("anomaly:".length),
    ).length;
  if (key.startsWith("wormhole:"))
    return tile.wormholes.filter(
      (value) => value === key.slice("wormhole:".length),
    ).length;
  switch (key) {
    case "redSystems":
      return tile.redSystems;
    case "anomalySystems":
      return tile.anomalySystems;
    case "wormholeSystems":
      return tile.wormholeSystems;
    case "specialtySystems":
      return tile.specialtySystems;
    case "legendarySystems":
      return tile.legendarySystems;
    case "emptySystems":
      return tile.emptySystems;
    case "onePlanetSystems":
      return tile.onePlanetSystems;
    case "twoPlanetSystems":
      return tile.twoPlanetSystems;
    case "threePlanetSystems":
      return tile.threePlanetSystems;
    default:
      return 0;
  }
}
...
export function availableFeatureKeys(tiles: SystemTile[]): FeatureKey[] {
  ...
  for (const tile of tiles) {
    ...
    Object.keys(tile.specialtyByType).forEach((value) =>
  ...
  return [
    ...
    ...[...specialties]
      .sort()
      .map((value) => `specialty:${value}` as FeatureKey),
    "legendarySystems",
    "emptySystems",
    "onePlanetSystems",
    "twoPlanetSystems",
...
export function featureRange(
  ...
  const counts = tiles
    .map((tile) => featureCount(tile, key))
    .sort((a, b) => a - b);
  return {
...
export function featureLabel(key: FeatureKey): string {
  ...
  if (key.startsWith("specialty:"))
    return `${SPECIALTY_LABELS[key.slice(10)] ?? key.slice(10)} icons`;
  if (key.startsWith("anomaly:")) return `${titleCase(key.slice(8))} systems`;
  if (key.startsWith("wormhole:"))
    return `${key.slice(9).toUpperCase()} wormholes`;
  return key;
}
...
export function tileImageUrl(tileId: string): string {
  return `${import.meta.env.BASE_URL}tiles/ST_${tileId}.webp`;
}
